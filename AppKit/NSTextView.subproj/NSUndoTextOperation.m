                          ...
                          text: (NSAttributedString *) string
{
    ...
    if ((selectedRange.length == 0) &&
        (selectedRange.location == NSMaxRange(_replacementRange)) &&
        (affectedRange.location == replacementRange.location)) {
        if (replacementRange.length > 0) {
            // Extending the replacement range - new text is just added
            _replacementRange.length += replacementRange.length;
            return YES;
        } else {
            // First try to shorten the replacement range (we're deleting some
            // chars previously inserted)
            if (_replacementRange.length > affectedRange.length) {
                _replacementRange.length -= affectedRange.length;
            } else {
                // Then, when empty, extend the affected range (we're attacking
