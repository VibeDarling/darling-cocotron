...
/* There is essentially only two ways to implement a CGLContext on Windows.
   ...
   1) A double buffered off-screen window, using glReadPixels on the back
   buffer. You can't use a single buffered window because the front buffer is
   typically considered on-screen and will more likely fail the pixel ownership
   test. You can't use the front buffer in a double buffered window for the same
   reasons.

     Technique 1 works on a lot of systems, but not all. Some drivers will
   consider an off-screen window as failing the pixel ownership test and not
   produce any results (black). Some nVidia card/drivers exhibit this behavior.
     Technique 1 is also slow and there are no acceleration options, the pixels
   must be retrieved with glReadPixels


   2) Two pbuffers. Because pbuffers can't be resized you must use one pbuffer
   for the rendering context and another which is destroyed/created for the
   backing. The one small static pbuffer is used as the primary rendering
   context and participates in wglShareLists and preserves rendering context
   state during resizes. A second pbuffer is used as the backing HDC, this
   pbuffer is destroyed/created during a context resize (pbuffers can't be
   resized), no HGLRC is needed for this pbuffer (although we create one as a
   side effect of pbuffer management). This is all possible because
   wglMakeCurrent accepts both a HDC and HGLRC, provided the HDC and HGLRC have
   the same pixel format.

   Technique 2 is used if pbuffers are available, 1 is the fallback.

   You can't use FBO's because they are context state and you can't hide them
   from the application software. E.g. if the application switches to fbo 0
   (default), and CGLContext is using 1 for the backing things will fail.
   Pbuffer doesn't have this problem, pbuffer is also older and more common on
   Windows at least.

   You can't use child windows (this was the first implementation) because they
   have a number of problems, e.g. no transparency, they are not supported well
   in layered windows, you can't have multiple child windows with different
   pixels formats and they have a number of visual artifacts which are hard to
   control, e.g. flicker during resize.

   VMWare Fusion 3 (as of April 4,2011) does not support pbuffers so you can't
   test it there. Parallels Desktop (as of April 4,2011) does support pbuffers
   but the implementation appears slightly buggy. There is a workaround present
   (see WARNING in the pbuffer resize code).
    */
...
/* This is a dedicated thread for creating and dispatching messages to OpenGL
   context windows.

   Each HWND in Win32 is associated with the thread it is created on. Messages
   for that window are sent to the associated threads input queue. So, for
   example if you send a MoveWindow to a window, all the underlying messages
   related to the MoveWindow are sent to the input queue on the associated
   thread.

   The problem is that the application developer can not be expected to process
   the messages on the creating thread. In a single threaded AppKit program
   under typical condition that is OK because you are processing messages, but
   in a multi-threaded environment where the creating thread is not handling
   messages this can be a problem.

   So, we create all OpenGL windows on a dedicated thread which handles the
   messages.
                                ...
                                GLint *w, GLint *h)
{
    if (parentWindow != nil) {
        ...
        CGNativeBorderFrameWidthsForStyle([parentWindow styleMask], &top, &left,
                                          &bottom, &right);
...
static int resizeBackingIfNeeded(CGLContextObj context) {
    ...
    /* If we're using a Pbuffer we don't want the window large because it
     * consumes resources */
    ...
    if (usesChildWindow(context) || (context->dynamicpBuffer == NULL)) {
        ...
        // Window context must be current for pBuffer functions to work. Since
        // this is now only called during a SetCurrent, no need to
        // wglMakeCurrent

        /* WARNING:
         *   We retain the old dynamic pbuffer here, create a new one, then
         * release the old one. There is a bug (mis-feature?) in the Parallels
         * driver at least which will cause invalid pbuffers to be returned if
         * you immediately free then alloc a new one. If you save the old one
         * and free it AFTER the new one is allocated it all works. I am not
         * aware of this being a problem in other drivers.
         *
         *   This can be problematic with big pbuffers as the old one would be
         * around while allocating a new one. Could possibly special case for
         * just Parallels as that appears to be the only place it happens.
         */
...
CGLError _CGLSetCurrentContextFromThreadLocal(int value) {
    ...
    if (context == NULL)
        ...
        if (usesChildWindow(context) || context->dynamicpBuffer == NULL) {
            ...
            opengl_wglMakeCurrent(context->dynamicpBuffer->dc,
                                  context->windowGLContext);
                               ...
                               CGLPixelFormatObj pixelFormat)
{
    ...
    /* It has to be double buffered regardless of what the application asks for,
       because we're reading from it, all the pixels must be valid. A single
       buffer context is problematic in that the driver may not render obscured
       pixels, all of them since it is off-screen. That isnt a problem with
       pbuffers but this is the fallback. */
...
void _CGLCreateDynamicPbufferBacking(CGLContextObj context) {
    ...
    if (context->dynamicpBuffer == NULL) {
        if (CGLCreatePBuffer(context->w, context->h, WGL_TEXTURE_2D_ARB,
                             WGL_TEXTURE_RGBA_ARB, 0,
                             &(context->dynamicpBuffer)) != kCGLNoError)
            return;

        if (CGLSetPBuffer(context, context->dynamicpBuffer, 0, 0, 0) !=
            kCGLNoError) {
                                     ...
                                     CGLContextObj *resultp)
{
    ...
    /* Creating a CGL context does NOT set it as current, if you need the
       context to be current, save it and restore it. Some applications may
       create a context while using another, so we don't want to improperly
       switch to another context during creating */
...
CGL_EXPORT CGLContextObj CGLRetainContext(CGLContextObj context) {
    // if(NSDebugEnabled) NSCLog("%s %d %s
    // %p",__FILE__,__LINE__,__PRETTY_FUNCTION__,context);
    if (context == NULL)
        return NULL;
                                    ...
                                    const GLint *value)
{
    ...
    case kCGLCPSurfaceBackingSize:;
        ...
        if (sizeChanged) {
            ...
            /* We immediately resize so that the resize happens on the thread
             * does the resize, otherwise MoveWindow can deadlock. */
                                             ...
                                             CGLContextObj destination)
{
    ...
    AlphaBlend([[dstSurface deviceContext] dc], 0, 0, width, height,
               [[srcSurface deviceContext] dc], 0, 0, width, height, blend);
                                  ...
                                  CGLContextObj destination)
{
    ...
    O2SurfaceLock(dstSurface);
    O2SurfaceLock(srcSurface);
    AlphaBlend([[dstSurface deviceContext] dc], source->x, y, source->w,
               source->h, [[srcSurface deviceContext] dc], 0, 0, source->w,
               source->h, blend);
    O2SurfaceUnlock(srcSurface);
    O2SurfaceUnlock(dstSurface);
                       ...
                       GLenum face, GLint level, GLint screen)
{
    ...
    int piAttribIList[] = {WGL_SUPPORT_OPENGL_ARB,
                           GL_TRUE,
    ...
    if (opengl_wglChoosePixelFormatARB(CGLGetWindowDC(context), piAttribIList,
                                       NULL, 1, piFormats,
                                       &nNumFormats) == FALSE) {
    ...
    opengl_wglQueryPbufferARB(pbuffer->pBuffer, WGL_PBUFFER_HEIGHT_ARB,
                              &height);
    reportGLErrorIfNeeded(__PRETTY_FUNCTION__, __LINE__);
